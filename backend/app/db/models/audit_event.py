...
class AuditEvent(Base):
    ...
    def verify_chain(self, previous_event: Optional['AuditEvent'] = None) -> bool:
        ...
        if self.sequence_number == 1:
            return self.previous_hash is None and self.event_hash == self.compute_hash()
        
        if self.sequence_number != 1 and previous_event is None:
            return False
        if previous_event and self.sequence_number != previous_event.sequence_number + 1:
            return False
