...
def evaluate(db, workflow_id, model):
    ...
    if switches: return False, 'Emergency stop is active: ' + str(switches[0].id)
    ...
    for policy in policies:
        if any(k in context and context[k] == v for k,v in (policy.auto_deny_conditions or {}).items()):
            return False, 'Automatic deny condition matched: policy ' + str(policy.id)
        ...
        if policy.policy_action != PolicyAction.ALLOW:
            return False, 'Policy denies execution or requires review: ' + str(policy.id)
        allowed = True
