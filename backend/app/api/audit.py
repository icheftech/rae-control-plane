...
def events(limit: int=Query(100, ge=1, le=500), db: Session=Depends(get_db)):
    return [e.to_dict() for e in db.scalars(select(AuditEvent).order_by(AuditEvent.sequence_number.desc()).limit(limit))]
@router.get('/verify')
def verify(db: Session=Depends(get_db)):
    ...
    for event in db.scalars(select(AuditEvent).order_by(AuditEvent.sequence_number)):
        if not event.verify_chain(previous):
            return {'valid':False, 'verified_count':count, 'failed_sequence':event.sequence_number}
        previous = event
        count += 1
