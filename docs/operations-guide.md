# R.A.E. Evaluation and Operations Guide

Setup, API behavior, validation, and release boundaries for the current R.A.E. implementation. For product positioning and adoption models, see the [platform overview](../README.md).

## Working features

- Southern Shade local SSO and content-free orchestration timelines: [setup and boundaries](local-sso-and-run-history.md).
...
Requires Python 3.12 (the CI-tested version), Node 20.9+, and Docker Desktop.
...
For a containerized API, run `docker compose up --build -d` and set the frontend's `NEXT_PUBLIC_API_URL` to `http://127.0.0.1:8000`. The image applies migrations before starting. The durable local worker runs as a separate process; it does not require Redis or Celery. See the [worker guide](local-worker.md).

## API

Interactive documentation: `http://127.0.0.1:18000/api/docs`.
Data endpoints require an authenticated API key or supported browser session. Service clients use `Authorization: Bearer <key>`. Health is public.
...
- `/api/local-jobs` and `/api/local-jobs/workers`
...
The optional [durable local worker](local-worker.md) adds a persistent job
...
Tests create a temporary PostgreSQL container, apply real Alembic migrations, and isolate most tests with savepoints. Worker concurrency and recovery tests use committed sessions in separate test tenants. The required 80% coverage threshold is retained.
