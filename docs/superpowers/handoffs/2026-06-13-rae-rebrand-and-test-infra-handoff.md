...
5. **Generate real Alembic migrations**
   ...
   - Replace stub `migrations/versions/001_initial_schema.py`
   - Switch test fixture from `Base.metadata.create_all()` to `alembic upgrade head`

6. **Code review of merged remote work** (tenants API, business eval scorer)
   - Run `/code-review` on `git diff e7043d2..977f437` (the remote branch that was merged)
   - Pay particular attention to `backend/app/api/tenants.py` and any new model files
